...
class MultiLevelCascadeEarlyExitSSDLite(nn.Module):
    ...
    def compute_confidence(self, cls_logits, temperature):
        """
        FIXED: Compute confidence using max probability and object-aware metrics
        This fixes the critical bug in Phase 1 where entropy-based confidence
        was always near-zero due to background class dominance.
        """
        ...
        # FIX #1: Use max probability instead of entropy
        # This gives the confidence of the top prediction
        max_probs, predicted_classes = probs.max(dim=-1)

        # FIX #2: Object-aware confidence
        # Separate confidence for background vs foreground predictions
        background_mask = (predicted_classes == 0)
        foreground_mask = (predicted_classes == 1)

        # Count predictions per class
        num_background = background_mask.sum(dim=1).float()
        num_foreground = foreground_mask.sum(dim=1).float()

        # Compute weighted confidence
        # If objects detected: weight foreground confidence higher
        # If no objects: use background confidence
        confidence_list = []
        for i in range(batch_size):
            if num_foreground[i] > 0:
                # Has object predictions - focus on those
                fg_conf = max_probs[i][foreground_mask[i]].mean()
                bg_conf = max_probs[i][background_mask[i]].mean()
                # Weight foreground 70%, background 30%
                conf = 0.7 * fg_conf + 0.3 * bg_conf
            else:
                # No objects - use background confidence
                conf = max_probs[i][background_mask[i]].mean()
...
def main():
    ...
    BATCH_SIZE = 4
    NUM_EPOCHS = 20
    EXIT1_THRESHOLD = 0.01  # Lower threshold for testing (was 0.60)
    EXIT2_THRESHOLD = 0.02  # Higher threshold for testing (was 0.75)
    NUM_WORKERS = 0
