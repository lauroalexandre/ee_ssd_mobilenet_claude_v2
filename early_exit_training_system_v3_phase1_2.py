# early_exit_training_system_v3_phase1_2.py
# Phase 1.2: Hybrid Approach with Detection-Aware Confidence and Real Targets
...
class MultiLevelCascadeEarlyExitSSDLite(nn.Module):
    ...
    def compute_confidence(self, cls_logits, temperature, bbox_regression=None):
        """
        Phase 1.2: Detection-aware confidence metric

        Combines:
        1. Objectness score (chair vs background)
        2. Classification confidence
        3. Bbox regression quality (if available)

        This provides a better metric for early exit decisions in object detection.
        """
        ...
        # DETECTION-AWARE CONFIDENCE:
        # Focus on predictions with high objectness (likely detections)
        objectness_threshold = 0.3  # Lower than inference to allow learning
        ...
        for i in range(batch_size):
            ...
            if object_mask.sum() > 0:
                # Has confident object predictions
                ...
                # Penalize confidence when no objects detected
                conf = conf * 0.5  # Lower confidence for background-only
...
def main():
    ...
    BATCH_SIZE = 4
    NUM_EPOCHS = 20
    # Phase 1.2: Detection-aware confidence thresholds
    # These are tuned for objectness-based confidence (not max probability)
    EXIT1_THRESHOLD = 0.35  # Easy detections (clear objects)
    EXIT2_THRESHOLD = 0.50  # Medium difficulty detections
    NUM_WORKERS = 0

    print("="*80)
    print(" Phase 1.2: Hybrid Approach with Detection-Aware Confidence")
    print("="*80)
    print()
