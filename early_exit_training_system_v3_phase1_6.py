...
class Trainer:
    ...
    def create_plots(self):
        ...
        axes[0, 1].axhline(y=self.model.exit1_threshold.cpu().item(), color='r', linestyle=':', alpha=0.5, label=f'Thresh 1 ({self.model.exit1_threshold.cpu().item():.3f})')
        axes[0, 1].axhline(y=self.model.exit2_threshold.cpu().item(), color='g', linestyle=':', alpha=0.5, label=f'Thresh 2 ({self.model.exit2_threshold.cpu().item():.3f})')
