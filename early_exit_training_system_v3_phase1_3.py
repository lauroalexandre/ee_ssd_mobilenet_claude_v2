# early_exit_training_system_v3_phase1_3.py
# Phase 1.3: Conservative Thresholds and Improved Confidence Calibration
#
# Key improvements from Phase 1.2:
# - Increased exit thresholds (Exit1: 0.35→0.65, Exit2: 0.50→0.80)
# - Raised objectness threshold (0.3→0.5) for more selective detection
# - Added minimum object count requirement for high confidence
# - Stronger penalties for background-only predictions
# - Better calibration to prevent premature exits
...
class MultiLevelCascadeEarlyExitSSDLite(nn.Module):
    ...
    def __init__(self, base_model, exit1_threshold=0.65, exit2_threshold=0.80):
        super().__init__()
    ...
    def compute_confidence(self, cls_logits, temperature, bbox_regression=None):
        """
        Phase 1.3: Conservative detection-aware confidence metric

        Improvements from Phase 1.2:
        - Raised objectness threshold (0.3 → 0.5) for more selective detection
        - Added minimum object count requirement (at least 3 confident objects)
        - Stronger penalties for background-only predictions
        - Better calibration to prevent premature exits

        This provides a more conservative metric that only produces high confidence
        when there are clear, strong object detections.
        """
        ...
        # PHASE 1.3: CONSERVATIVE DETECTION-AWARE CONFIDENCE
        # Increased objectness threshold for more selective detection
        objectness_threshold = 0.5  # Increased from 0.3
        min_confident_objects = 3   # Require at least 3 confident detections
        ...
        for i in range(batch_size):
            ...
            if num_confident_objects >= min_confident_objects:
                ...
                # If we have exactly min_confident_objects, apply 0.8x penalty
                # Scale up linearly to 1.0x at 10+ objects
                object_count_factor = min(1.0, 0.8 + 0.2 * (num_confident_objects - min_confident_objects) / 7.0)
                ...
                # Strong penalty for insufficient object count
                conf = conf * 0.4  # Max 40% confidence with insufficient objects
                ...
                # Strong penalty for background-only predictions
                conf = conf * 0.3  # Max 30% confidence for background-only
    ...
    def prepare_targets_for_loss(self, cls_logits, bbox_regression, targets):
        """
        Phase 1.2: Prepare ground truth targets for SSD-style loss computation
    ...
    def compute_training_losses(self, exit1_cls, exit1_reg, exit1_conf,
        ...
        """
        Phase 1.2: Compute multi-level training losses with REAL ground truth targets

        Key changes from Phase 1.1:
        - Uses actual chair bounding boxes and labels from dataset
        - Proper SSD-style target assignment
        - Combines supervised loss with distillation
        """
...
class Trainer:
    ...
    def save_final_results(self):
        """Save final model and metrics"""
        # Save model
        final_model_path = os.path.join(MODEL_OUTPUT_PATH, 'early_exit_ssdlite_phase1_3_final.pth')
        torch.save(self.model.state_dict(), final_model_path)
        print(f"\nFinal model saved: {final_model_path}")

        # Save metrics to CSV
        train_df = pd.DataFrame(self.train_metrics)
        train_df.to_csv(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'train_metrics_phase1_3.csv'), index=False)

        val_df = pd.DataFrame(self.val_metrics)
        val_df.to_csv(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'val_metrics_phase1_3.csv'), index=False)
    ...
    def create_plots(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'training_plots_phase1_3.png'), dpi=150, bbox_inches='tight')
        plt.close()
...
def main():
    """Main training function for Phase 1.3"""
    # Configuration
    BATCH_SIZE = 4
    NUM_EPOCHS = 20
    # Phase 1.3: Conservative thresholds to prevent premature exits
    # Significantly increased from Phase 1.2 to ensure proper cascade behavior
    EXIT1_THRESHOLD = 0.65  # Increased from 0.35 - only very confident detections
    EXIT2_THRESHOLD = 0.80  # Increased from 0.50 - high confidence required
    NUM_WORKERS = 0

    print("="*80)
    print(" Phase 1.3: Conservative Thresholds & Improved Confidence Calibration")
    print("="*80)
    print()
    print("KEY IMPROVEMENTS FROM PHASE 1.2:")
    print("  - Exit 1 Threshold: 0.35 → 0.65 (86% increase)")
    print("  - Exit 2 Threshold: 0.50 → 0.80 (60% increase)")
    print("  - Objectness Threshold: 0.3 → 0.5 (67% increase)")
    print("  - Minimum Object Count: Added requirement for 3+ confident objects")
    print("  - Stronger penalties for background-only predictions")
    print("="*80)
    print()
    ...
    # Initialize model
    print("Initializing Phase 1.3 model...")
    ...
    # Save final summary
    with open(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'final_summary_phase1_3.json'), 'w') as f:
        json.dump(final_metrics, f, indent=4)

    # Print comprehensive summary
    print("="*80)
    print(" PHASE 1.3 FINAL RESULTS")
    print("="*80)
    print()
    ...
    phase1_2_early_exit_rate = 1.0  # 100% from Phase 1.2
    ...
    print(f"  Phase 1.2 Early Exit Rate:      {phase1_2_early_exit_rate:.2%} (TOO HIGH - all exit at E1)")
    print(f"  Phase 1.3 Early Exit Rate:      {final_metrics['final_combined_early_exit_rate']:.2%}")
    print()
    print("  GOAL: Achieve balanced distribution across exits")
    print("        (e.g., 20-30% Exit1, 20-30% Exit2, 40-60% Full)")
    print("="*80)
