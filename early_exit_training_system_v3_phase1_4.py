# early_exit_training_system_v3_phase1_4.py
# Phase 1.4: Moderate Achievable Thresholds for Balanced Exit Distribution
#
# Key improvements from Phase 1.3:
# - Reduced exit thresholds to ACHIEVABLE levels (Exit1: 0.65→0.45, Exit2: 0.80→0.60)
# - Relaxed objectness threshold (0.5→0.4) for less restrictive detection
# - Reduced minimum object count requirement (3→2) for more flexibility
# - Goal: Achieve balanced distribution (20-40% Exit1, 20-30% Exit2, 30-60% Full)
#
# Phase 1.3 Analysis Findings:
# - Model confidence levels: Exit1=0.486, Exit2=0.521, Full=0.636
# - Thresholds 0.65/0.80 were TOO HIGH (0% early exit rate)
# - Model architecture and cascade learning work correctly
# - Only needed: threshold calibration to match observed confidence
...
class Trainer:
    ...
    def save_final_results(self):
        """Save final model and metrics"""
        # Save model
        final_model_path = os.path.join(MODEL_OUTPUT_PATH, 'early_exit_ssdlite_phase1_4_final.pth')
        torch.save(self.model.state_dict(), final_model_path)
        print(f"\nFinal model saved: {final_model_path}")

        # Save metrics to CSV
        train_df = pd.DataFrame(self.train_metrics)
        train_df.to_csv(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'train_metrics_phase1_4.csv'), index=False)

        val_df = pd.DataFrame(self.val_metrics)
        val_df.to_csv(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'val_metrics_phase1_4.csv'), index=False)
    ...
    def create_plots(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'training_plots_phase1_4.png'), dpi=150, bbox_inches='tight')
        plt.close()
...
def main():
    """Main training function for Phase 1.4"""
    # Configuration
    BATCH_SIZE = 4
    NUM_EPOCHS = 20
    # Phase 1.4: Moderate achievable thresholds for balanced exit distribution
    # Based on Phase 1.3 observed confidence levels (Exit1=0.486, Exit2=0.521, Full=0.636)
    EXIT1_THRESHOLD = 0.45  # Just below observed 0.486 - allows ~20-40% early exit
    EXIT2_THRESHOLD = 0.60  # Between observed 0.521 and 0.636 - allows ~20-30% exit here
    NUM_WORKERS = 0

    print("="*80)
    print(" Phase 1.4: Moderate Achievable Thresholds for Balanced Exits")
    print("="*80)
    print()
    print("KEY IMPROVEMENTS FROM PHASE 1.3:")
    print("  - Exit 1 Threshold: 0.65 → 0.45 (31% decrease, now ACHIEVABLE)")
    print("  - Exit 2 Threshold: 0.80 → 0.60 (25% decrease, now ACHIEVABLE)")
    print("  - Objectness Threshold: 0.5 → 0.4 (20% decrease)")
    print("  - Minimum Object Count: 3 → 2 (more flexible detection)")
    print("  - Thresholds calibrated to observed confidence levels from Phase 1.3")
    print("="*80)
    print()
    print("PHASE 1.3 ANALYSIS:")
    print("  - Observed Exit 1 confidence: 0.486 (threshold was 0.65 - TOO HIGH)")
    print("  - Observed Exit 2 confidence: 0.521 (threshold was 0.80 - TOO HIGH)")
    print("  - Observed Full confidence:  0.636")
    print("  - Result: 0% early exit rate (overcorrection)")
    print("="*80)
    print()
    print("PHASE 1.4 GOAL:")
    print("  - 20-40% exit at Exit 1 (easy cases)")
    print("  - 20-30% exit at Exit 2 (moderate cases)")
    print("  - 30-60% reach Full Model (complex cases)")
    print("="*80)
    print()
    ...
    # Initialize model
    print("Initializing Phase 1.4 model...")
    ...
    print("\n" + "="*80)
    print(" Training Complete - Phase 1.4")
    print("="*80)
    ...
    # Save final summary
    with open(os.path.join(ANALYSIS_EE_OUTPUT_PATH, 'final_summary_phase1_4.json'), 'w') as f:
        json.dump(final_metrics, f, indent=4)

    # Print comprehensive summary
    print("="*80)
    print(" PHASE 1.4 FINAL RESULTS")
    print("="*80)
    print()
    ...
    print(f"  Training:")
    print(f"    Exit 1:  {final_metrics['train_exit1_confidence']:.3f} (threshold: {EXIT1_THRESHOLD})")
    print(f"    Exit 2:  {final_metrics['train_exit2_confidence']:.3f} (threshold: {EXIT2_THRESHOLD})")
    ...
    phase1_3_early_exit_rate = 0.0  # 0% from Phase 1.3 (overcorrection)
    phase1_2_early_exit_rate = 1.0  # 100% from Phase 1.2 (undercorrection)
    ...
    print(f"  Phase 1.3 Early Exit Rate:      {phase1_3_early_exit_rate:.2%} (TOO LOW - none exit early)")
    print(f"  Phase 1.4 Early Exit Rate:      {final_metrics['final_combined_early_exit_rate']:.2%} (TARGET)")
    print()
